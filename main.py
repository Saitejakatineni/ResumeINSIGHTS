...
Run dev server: uvicorn main:app --reload --port 8765
Launch desktop app: python launcher.py
"""
...
# Supports both dev (.env in project root) and installed app
# (~/Library/Application Support/resumeINSIGHTS/.env via env var)
...
# Sonnet pricing (as of 2025-03): $3/M input, $15/M output
    ...
    with get_db() as con:
        cur = con.execute(
            """INSERT INTO analyses
               (created_at, resume_name, jd_snippet, jd_full, result_json)
               VALUES (?, ?, ?, ?, ?)""",
            (
...
def get_history() -> list[dict]:
    with get_db() as con:
        rows = con.execute(
            """SELECT id, created_at, resume_name, jd_snippet, result_json
               FROM analyses ORDER BY id DESC LIMIT 50"""
        ).fetchall()

    # FIX: parse result_json once per row instead of 3 times
    items = []
    for r in rows:
...
def build_messages(file_bytes: bytes, filename: str, job_description: str) -> list:
    """Build the Claude API messages list for a PDF or DOCX resume."""
...
def parse_json_from_response(text: str) -> dict:
    """Extract and parse the JSON object from the model's response text."""
...
async def test_analysis():
    ...
    result = {
        ...
        "overall_grade": "B-",
        "match_summary": (
            "The candidate has a solid Python and data engineering background that partially "
            "aligns with the role. Key cloud and pipeline tools from the JD (DBT, Snowflake, "
            "Airflow) are absent from the resume, which will likely cause ATS filtering. "
            "With targeted additions the match can reach 90%+."
        ),
    ...
    result["history_id"] = save_analysis("sample_resume.pdf", "Sample job description for testing.", result)
...
async def analyze_resume(
    ...
    try:
        messages = build_messages(content, filename, job_description)
        ...
        print(f"[analyze] Done — input={input_tokens} output={output_tokens} est_cost=${cost_usd:.4f}")
        ...
        result["history_id"] = save_analysis(
            resume.filename or "resume", job_description, result
        )

        return JSONResponse(result)

    except (HTTPException, anthropic.APIError):
        raise
    except json.JSONDecodeError:
...
# Serve the frontend — must be registered last so API routes take priority
